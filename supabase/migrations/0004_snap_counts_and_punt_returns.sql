-- Fixes a real bug: team_game_stats.offense_snaps/defense_snaps were
-- SUM'd across every player on the roster for that game -- ~11 players each
-- playing ~65 snaps sums to ~700+, not the team's actual play count. The
-- correct proxy is MAX (the most-used player, e.g. a starting lineman or
-- QB, played essentially every snap the offense/defense ran that game).
--
-- Also adds punt-return-rate tracking (pt_returned = punts of ours that
-- were returned by the opponent, punt_returns = opponent punts we
-- returned), and changes punts from a season SUM to a per-game AVG to
-- match every other rate stat in team_season_stats.
...
select
    ...
    -- % of this team's own punts that the opponent returned, and % of the
    -- opponent's punts (against this team) that this team's returner ran back
